...
function ENT:StuckThink()
	...
	if not self:IsInWorld() and self:GetClass() != "npc_hum_anarchist" then
			RemoveRemaining( 1 )
			self.Entity:Remove()
	end
...
function ENT:OnInjured( dmginfo )
	...
	self.Entity:AddDamageTaken( dmginfo:GetAttacker(), dmginfo:GetDamage() * 3 )
	else
...
function ENT:CheckOK( name ) --Put those bugged doors in here. You know the ones

	local badshit = ents.FindByName( "bm*")
	badshit = table.Add( badshit, ents.FindByName( "bf*") )
	badshit = table.Add( badshit, ents.FindByName( "bomb*") )
	badshit = table.Add( badshit, ents.FindByName( "cslid*") )
	badshit = table.Add( badshit, ents.FindByName( "shopd*") )
	badshit = table.Add( badshit, ents.FindByName( "psmaindoor") )
		...
		if not ent.Hits then
			...
			if ent.Hits > ent.MaxHits then
				...
				if ent:GetModel() != "models/props_debris/wood_board04a.mdl" and ent:GetName() != "ghost_stuff" then
					...
					if self:CheckOK( ent:GetName() ) then
						ent:Remove()
					else
...
function ENT:BehaveAct() // what does this even?
...
function ENT:CanTarget( v )
	...
	return ( ( v:IsPlayer() and v:Alive() and v:GetObserverMode() == OBS_MODE_NONE and v:Team() == TEAM_ARMY and v:IsCloaked() == false ) or ( v.NextBot and not v:IsZombie() ) or ( v.IsWood ) )
...
function ENT:GetBreakable()
	...
	tbl = table.Add( tbl, ents.FindByClass( "prop_door*" ) )
	tbl = table.Add( tbl, ents.FindByClass( "func_breakable*" ) )
	tbl = table.Add( tbl, ents.FindByName( "func_door*" ) )
	 
	for k,v in pairs( tbl ) do
	
		if IsValid( v ) and v:GetPos() != Vector(0,0,0) and v:GetPos():Distance( self.Entity:GetPos() ) <= self.BreakableDistance and v:GetName() != "ghost_stuff" then
	...
	if table.HasValue( GAMEMODE.Breakables, tr.Entity ) and IsValid( tr.Entity ) and tr.Entity:GetName() != "ghost_stuff" then
...
function ENT:BreakableRoutine()
	...
	while IsValid( ent ) and ent:GetName() != "ghost_stuff"  do
