...
function SWEP:BarricadeTrace()
	...
	if self.Owner:GetPlayerClass() == 4 then
	local ghostcount = #ents.FindByName( "ghost_stuff" )
	if ghostcount > 0 then
	for k,v in pairs( ents.FindByName( "ghost_stuff" ) ) do
	local dist = self.Weapon:GetOwner():GetPos() - v:GetPos()
	dist = dist:Length()
	if dist < 70 then
		v.MaxHits = 10 + (self.Weapon:GetOwner():GetLevel() * 2 )
		v.Hits = 0
		...
		v:SetNotSolid( false )
		self.Owner:Notice( "Rebuilt a door using 1 piece of wood", GAMEMODE.Colors.Green )
	...
	if tr.HitWorld and trlength < 150 then
		...
		self.Owner:NoticeOnce( "You can also repair and rebuild doors", GAMEMODE.Colors.Blue, 5, 2 )
		
		local prop = ents.Create( "prop_physics" )
		prop:SetModel( self.Barricade )
		prop:Spawn()
		prop:SetHealth( 350 + (self.Weapon:GetOwner():GetLevel() * 15 ) )
		prop:SetCustomCollisionCheck()
		prop.IsWood = true
		
		self.Weapon:SetPlacePosition( prop )
			
	elseif string.find( tr.Entity:GetClass(), "prop_door" ) and trlength < 100 then
		
		self.Owner:Notice( "Fortified a door using 1 piece of wood", GAMEMODE.Colors.Green )
		if self.Owner:GetPlayerClass() == CLASS_ENGINEER then
		tr.Entity.MaxHits = 30 + (self.Weapon:GetOwner():GetLevel() * 3 )
		else
		tr.Entity.MaxHits = 25
		end
		tr.Entity.Hits = 0
