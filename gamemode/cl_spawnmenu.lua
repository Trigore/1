...
{ Name = "Supplies", Icon = "icon16/pill.png", Categories = { ITEM_SUPPLY, ITEM_SPECIAL, ITEM_ENGI } },
	...
	if style == "Buy" then
		...
		for k,v in pairs( tbl.Functions ) do
			...
			if count >= v then
				...
				btn:SetPos( 510, ypos )
				btn:SetSize( 250, 30 )
				btn:SetImage( "icon16/application.png" )
					
				if v == 1 then
					btn:SetText( "Sell" )
				else
					btn:SetImage( "icon16/application_view_tile.png" )
					btn:SetText( "Sell  " .. v )
				end
					
				btn:SetFunction( function() RunConsoleCommand( "inv_sell", tbl.ID, v ) end )
		...
		if count > 1 then
			...
			btn:SetPos( 510, ypos )
			btn:SetSize( 250, 30 )
			btn:SetImage( "icon16/application_cascade.png" )
			btn:SetText( "Sell  All"	)
			btn:SetFunction( function() RunConsoleCommand( "inv_sell", tbl.ID, count ) end )
...
function StoreMenu( msg )
	...
	if open then
		...
		local CloaseButton = GAMEMODE:CreateElement( "CategoryButton" )
		CloaseButton:SetPos( 510, ScrH() - 115 )
		CloaseButton:SetText( "Close Trader" )
		CloaseButton:SetImage( "icon16/car.png" )
		CloaseButton:SetSize( 250, 30 )
		CloaseButton:SetSelectedState( true, true )
		CloaseButton.OnMousePressed = function() 
		surface.PlaySound( table.Random( GAMEMODE.RadioBeep ) )
		RunConsoleCommand( "closetrader" ) 
		end
