...
include( 'vgui/vgui_classscout.lua' )
include( 'vgui/vgui_classcomm.lua' )
include( 'vgui/vgui_classspec.lua' )
include( 'vgui/vgui_classengi.lua' )
include( 'vgui/vgui_cheatcodes.lua' )



CV_RagdollVision = CreateClientConVar( "cl_toxsinx_ragdoll_vision", "1", true, false )
CV_Density = CreateClientConVar( "cl_toxsinx_rain_density", "1.0", true, false )
CV_NoobHelp = CreateClientConVar( "cl_toxsinx_noob_help", "1", true, false )


function GM:ClassBoard( ply )

	local ref = ply:GetNWInt( "Class", 10 )
...
function GM:DrawMarkers()
	...
	tbl = table.Add( tbl, ents.FindByClass( "sent_hijackflare" ) )
	tbl = table.Add( tbl, ents.FindByClass( "sent_antidote" ) )
	tbl = table.Add( tbl, ents.FindByClass( "sent_trader" ) )
	...
	for k,v in pairs( tbl ) do
		...
		if sc.visible then
			...
			if v:GetClass() == "sent_hijackflare" then
			...
			if v:GetClass() == "sent_trader" then
			
				text = "trader"
				offset = ( v:GetPos() + Vector(0,0,50) ):ToScreen()
				maxdist = 100
...
function GM:HUDPaint()
	...
	local matClass = Material( "radbox/img_health" )
		if LocalPlayer():GetNWInt( "Class", 1 ) == 1 then 
		matClass = Material( "radbox/img_scout" )
		end
		if LocalPlayer():GetNWInt( "Class", 1 ) == 2 then 
		matClass = Material( "radbox/img_commando" )
		end
		if LocalPlayer():GetNWInt( "Class", 1 ) == 3 then 
		matClass = Material( "radbox/img_specialist" )
		end
		if LocalPlayer():GetNWInt( "Class", 1 ) == 4 then 
		matClass = Material( "radbox/img_engineer" )
		end
		if LocalPlayer():GetNWInt( "Class", 1 ) == 5 then 		 
		matClass = Material( "radbox/img_infect" )
		end
	if GetGlobalBool( "GameOver", false ) then return end
	local start = GetGlobalBool( "Start", false )
	local waveend = GetGlobalBool( "WaveEnd", true )
	...
	if not LocalPlayer():Alive() and LocalPlayer():Team() != TEAM_UNASSIGNED then
		...
		if DeathScreenScale == 1 then
			...
			if ( GetGlobalBool( "WaveEnd", false ) == false ) and ( GetGlobalBool( "Start", true ) == true ) then
			
				draw.SimpleText( "YOU WILL RESPAWN WHEN THE WAVE ENDS", "DeathFont", ScrW() * 0.5, ScrH() * 0.1, Color( 255, 0, 0 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
				
			end
			
			if ( GetGlobalBool( "Start", false ) == false ) or ( GetGlobalBool( "WaveEnd", true ) == true ) then
			
				draw.SimpleText( "PRESS YOUR JUMP BUTTON TO RESPAWN", "DeathFont", ScrW() * 0.5, ScrH() * 0.1, Color( 255, 0, 0 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			
			end
			
			draw.SimpleText( DeathScreenText or "POOP", "DeathFont", ScrW() * 0.5, ScrH() * 0.95, Color( 255, 0, 0 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			if LocalPlayer():Team() == TEAM_ZOMBIES and IsValid( LocalPlayer():GetObserverTarget() ) then
			draw.SimpleText( "Spectating: " .. LocalPlayer():GetObserverTarget():Nick(), "DeathFont", ScrW() * 0.5, ScrH() * 0.9, Color( 255, 0, 0 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			end
		end
	...
	DrawBar( xpos, ypos, xlen, ylen, LocalPlayer():Health(), LocalPlayer():GetMaxHealth(), matHealth, Color( 225, 223, 0, 255 ), Color( 252, 194, 0, 255 ), true )
	
	local pexp = LocalPlayer():GetNWInt( "Experience", 0 ) 
	local preq = LocalPlayer():GetNWInt( "ReqExperience", 25000 ) 
	
	if LocalPlayer():GetNWInt( "Level", 0 ) == 0 then
	pexp = math.Round(pexp / 167 )
	preq = math.Round(preq / 167 )
	end
	if LocalPlayer():GetNWInt( "Level", 0 ) == 1 then
	pexp = math.Round(pexp / 667 )
	preq = math.Round(preq / 667 )
	end
	if LocalPlayer():GetNWInt( "Level", 0 ) == 2 then
	pexp = math.Round(pexp / 3334 )
	preq = math.Round(preq / 3334 )	
	end
	if LocalPlayer():GetNWInt( "Level", 0 ) == 3 then
	pexp = math.Round(pexp / 10000 )
	preq = math.Round(preq / 10000 )	
	end
	if LocalPlayer():GetNWInt( "Level", 0 ) == 4 then
	pexp = math.Round(pexp / 23334 )
	preq = math.Round(preq / 23334 )	
	end
	if LocalPlayer():GetNWInt( "Level", 0 ) == 5 then
	pexp = math.Round(pexp / 36667 )
	preq = math.Round(preq / 36667 )	
	end
	local xposition = tonumber(ScrW() / 2)
	local yposition = 5
	local ylength = 20
	local xlength = 200
	DrawBar( xposition - preq, yposition, xlength, ylength, pexp, preq, matClass, Color( 0, 204, 0 ), Color( 0, 204, 0 ), false )
	draw.DrawText( LocalPlayer():GetNWInt( "Experience", 0 ) .. " / " .. LocalPlayer():GetNWInt( "ReqExperience", 25000 ),ExpXP,xposition + 20, yposition + 19.75,Color(0, 204, 0),TEXT_ALIGN_CENTER)
	draw.DrawText("Level " .. LocalPlayer():GetNWInt( "Level", 0 ),ExpXP,xposition - preq + 10,yposition + 19.75,Color( 0, 204, 0 ),TEXT_ALIGN_CENTER)
	ypos = ScrH() - 10 - ylen * 2
	
	local stam = LocalPlayer():GetNWInt( "Stamina", 0 )
	
	DrawBar( xpos, ypos, xlen, ylen, stam, 150, matStamina, Color( 138, 43, 226, 255 ), Color( 111, 0, 255, 255 ), false )
	...
	local remain = GetGlobalInt( "Remaining", 0 )
	local currentw = GetGlobalInt( "CurrentW", 0 )
	local totalw = GetGlobalInt( "TotalW", 0 )
	local display2 = "Wave " .. currentw .. " / " .. totalw
	local display = "Remaining: ".. remain
	if GetGlobalBool( "Start", false ) == false then
		display2 = "YOU ARE"
		if LocalPlayer():GetNWBool( "Ready", false ) then
		display = "READY"
		else
		display = "NOT READY"
		end
	else
		if GetGlobalBool( "Finale", false ) == true then
			display2 = "EVAC CALLED"
			else
		if GetGlobalBool( "WaveEnd", false ) == true then
		
			display2 = "TRADER OPEN"
		end
		end
		if remain == 0 then
			display = "SUSTAIN"
			if #ents.FindByClass( "sent_trader" ) == 1 then
			display = math.Round( GetGlobalInt( "TraderTime", 90 ) - CurTime() ) 
				if display >= 70 then
					display = "1:" .. math.Round( GetGlobalInt( "TraderTime", 90 ) - CurTime() - 60 )
				elseif display < 70 and display > 60 then
					display = "1:0" .. math.Round( GetGlobalInt( "TraderTime", 90 ) - CurTime() - 60 )
				elseif display == 60 then
					display = "1:00"
				else
					if display < 10 then
					display = "0:0" .. math.Round( GetGlobalInt( "TraderTime", 90 ) - CurTime() ) 
					else
					display = "0:" .. math.Round( GetGlobalInt( "TraderTime", 90 ) - CurTime() ) 
					end
				end
			end
			
		elseif remain > 99999 then
			display = "SURVIVE"
		end
	end
	local xposc = tonumber(ScrW())
	local yposc = tonumber(ScrH())/9 + 5
	local yc = 40
	local xc = 120
	DrawCash( xposc - xc, yposc - yc - yc, xc, yc, display2 )
	DrawCash( xposc - xc, yposc - yc, xc, yc, display)
...
function DeathScreen( msg )
	...
	DeathScreenScale = 0
	DeathScreenTime = CurTime() + 9999999
...
net.Receive( "LevelSynch", function( len )

	LocalLevel = net.ReadTable()
