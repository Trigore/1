...
function GM:GetPlayerStats( ply )
	...
	if not ply then 
		...
		return { "Deaths", "Kills", GAMEMODE.CurrencyName .. "s", "Level", "Class"} 
		
	else
	
		return { ply:GetNWInt( "Deaths", 0 ), ply:Frags(), ply:GetNWInt( "Cash", 10 ), ply:GetNWInt( "Level", 0 ), GAMEMODE:ClassBoard( ply ) } 
